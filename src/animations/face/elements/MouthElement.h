...
public:
	...
	void render(MouthType type, uint16_t foreground, uint16_t background, bool _drawBackgroundColor);
	void drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap,
					int16_t w, int16_t h, uint16_t color, uint16_t bg);
	bool drawBackgroundColor = false;
	FrameInfo getFrameInfo(MouthType type, uint16_t foreground, uint16_t background, bool _drawBackgroundColor);
